...
var ThemeSetupTool = (function($){
    ...
    function activateLicenseHandler(btn){
        var license_key = $('#' + tmc_theme_setup.theme_slug + '_license_key').val();
        //alert(license_key);
        var licenseStatus = $('.tmc-license-status');
        //force blur trigger
        $('#coderplace-license-activate .form-table').blur();
        loadingContent();
        var ajax = $.ajax({
            url: tmc_theme_setup.ajaxurl, 

            type: 'post',
            data: {
                    action: 'tmc_activate_theme_key',
                    wpnonce: tmc_theme_setup.wpnonce,
                    coderplace_license_activate: true,
                    coderplace_nonce : $(btn).data('nonce'),
                    key: license_key,
                    attempt: attempt || 0
                },
            ...
            complete: function(response){
                    ...
                    if( 'undefined' != response.done && 1 == response.done && 'undefined' != response.valid && 1 == response.valid ){
                        ...
                        $('.tmc-licenseinput').addClass( 'tmc-licenseinput--valid' );
                        ...
                        if(true===response.retry){
                            ...
                            $('.tmc-licenseinput').addClass( 'tmc-licenseinput--invalid' );

                            //unblock the page
                            unblockContent();

                            $('.tmc-licenseinput').on( 'click', function(){
    ...
    function clearLicenseMessage(){
        ...
        var licenseEntry = $('.tmc-licenseinput');
        licenseEntry.removeClass('tmc-licenseinput--valid');
        licenseEntry.removeClass('tmc-licenseinput--invalid');
    }
    ...
    function windowLoaded(){
        ...
        $('input.tmc-licenseinput__input').change(function() {
            clearLicenseMessage();
        });
    }

    function loadingContent(){
        $('.tmc-setupwizard__content').block({
            message: null,
            overlayCSS: {
                background: '#fff',
                opacity: 0.6
    ...
    function unblockContent(){
        $('.tmc-setupwizard__content').unblock({});
    }
    ...
    function PluginManager(){
        ...
        function ajaxCallback(response){
            if(typeof response == 'object' && typeof response.responseJSON != 'undefined'){
                    ...
                    if(response.hash == currentItemHash){
                        ...
                        $.ajax({
                            ...
                            complete: function(response2) {
                                        processCurrent();
                                        currentNode.find('span').text(response.message + tmc_theme_setup.verify_text);
                                    },
                            fail: ajaxCallback
                        });
                ...
                currentNode.find('span').text("Skipped Plugin Setup");
                console.log('ajax error 205');
                findNext();
            }
        }
        function processCurrent(){
            if(currentItem){
                ...
                if(required.is(':checked')) {
                    ...
                    $.ajax({    url: tmc_theme_setup.ajaxurl, 
                                type: 'post', 
                                data: {
                                    action: 'tmc_setup_plugins',
                                    wpnonce: tmc_theme_setup.wpnonce,
                                    slug: currentItem
                                },
        ...
        function findNext(){
            ...
            var li = $('.tmc-plugin-li .tmc-plugin-item');
            li.each(function(){
    ...
    function ContentManager(){
        ...
        function ajaxCallback(response) {
            if(typeof response == 'object' && typeof response.responseJSON != 'undefined'){
                ...
                currentNode.find('span').text("ajax error 319");
                findNext();
            }
        }
